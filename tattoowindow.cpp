...
TattooWindow::TattooWindow(bool open, QWidget *parent) : QWidget(parent), ui(new Ui::TattooWindow)
{
    // Setup
    ui->setupUi(this);
    ...
    QString err = QString();
    if (open) {
        location = QFileDialog::getOpenFileName(this, "Open tattoo mod", "./", "(*.xml)");
        ...
        if (!data.read_file(":/res/data_files/tattoo_default.xml", err)) {
            Utility::error(err);
